...
class sitemapSource {
	...
	static function _getDateModifiedSource(){
		...
		$query = "SELECT url, dateModified FROM `opiates` order by url where priority!=0";
	...
	static function dbSource(){
		static $qryResults;
		if(isset($qryResults)){
		   return $qryResults;
		}
		$query = "SELECT url, dateModified FROM `opiates` where priority!=0 order by url";
		$conn = databaseConnection::_getConnection();
		if(isset($query)){
	...
	static function dbSourceHTML(){
		static $qryResults;
		if(isset($qryResults)){
		   return $qryResults;
		}
		$query = "SELECT url, dateModified, uri, anchor FROM `opiates` where url!='/' && priority!=0 order by url ASC, priority DESC";
		$conn = databaseConnection::_getConnection();
		if(isset($query)){
		  $qryResults = mysqli_query( $conn, $query);
		  if(! ($qryResults && mysqli_num_rows($qryResults))){
	...
	public function setDateModified(){
		...
		$i=0;
		$i = sprintf('%04s', $i);
		while($row = mysqli_fetch_assoc($dateModified)){
			$i = sprintf('%04s', $i);
