...
class opiatePage extends page{
	...
	public function printGenericDrug($label=''){
		...
		if(!($this->genericDrugSource && mysql_num_rows($this->genericDrugSource))){
		  ...
		  if(mysql_num_rows($this->genericDrugSource)==1){
		  }elseif(mysql_num_rows($this->genericDrugSource)>1){
			  mysql_data_seek($this->genericDrugSource, 1);
		  }
...
/*	public function printRecentBlogPosts(){
	...
	public function setH1(){
		if(!($this->opiatePageSource && mysql_num_rows($this->opiatePageSource))){
		  return false;
		}
		else{
			if($row = mysql_fetch_assoc($this->opiatePageSource)){
				//print_r($row);
		...
		mysql_data_seek($this->opiatePageSource, 0);
	}
