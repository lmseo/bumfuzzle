...
require_once( $_SERVER['DOCUMENT_ROOT'] . '/includes/drug-master-9-1-11/widgets/class.breadcrumb.php');
class page{
	private $dbTable;
	private $pageSource, $breadCrumbSource;
	private $head;
	private $body;
		private $H1, $H2, $content, $dateModified, $dateCreated, $folderURL, $folderName, $canonical, $anchor;
	private $breadcrumb;
	private $isThereSource;

	public function __construct($table){
		$this->isThereSource=true;
		$this->H1 = $this->H2 =  $this->content = $this->dateModified= $this->dateCreated= $this->folderURL = $this->folderName = $this->canonical = '';
		$this->dbTable = $table;
		if(!($this->pageSource = source::_getPageSource($this->dbTable))){
	...
	public function setHead(){
		if(!($this->pageSource && mysql_num_rows($this->pageSource))){
		  return false;
		}
		else{
			if($row = mysql_fetch_assoc($this->pageSource)){
