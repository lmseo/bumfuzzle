<?php
class searchsToHTML extends toHTML{
	private $listHTML, $opiateListHTMLView;
	...
	public function getUnorderedList($row, $listItemClass='rightBox', $noLinkListItemClass='no-link'){
		$recentHTML = new html('ul');
		$recentHTML->set('class',$listItemClass);
		if($row['url'] != $_SERVER['REQUEST_URI'] && !empty($row['url']) && isset($row['url'])){
			if(basename($row['url']) == 'index.html'){
	...
	/*view for opiate list*/
	public function getOpiateListHTMLView(){
		if(isset( $this->opiateListHTMLView)){
			return $this->opiateListHTMLView;
		}else{
			return false;
		}
	}
	public function setOpiateListHTMLView($source, $container ){
		$this->opiateListHTMLView = new html($container);
		$numberOfElements=0;
		$recentHTML = new html('ul');
		while ($row = mysqli_fetch_assoc($source)){
			if(!($row && count($row))){
				...
				if($row['ancestor']== 222 && $row['descendant']==222 && length == 0){
					if($row['ancestor']== 222 && length == 1){
						$newList = $this->getUnorderedList($row);;
					}
					$listItem = $this->getAList($row);
					if(!$listItem){
					}else{
						$recentHTML->inject($listItem);
					}
		...
		if($numberOfElements > 0){
			$this->opiateListHTMLView->inject($recentHTML);
		}else{
			return false;
		}
