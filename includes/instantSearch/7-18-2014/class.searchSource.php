...
where t.termID=775))";
	...
	static function _getRandomRelatedTestimonialFomDB(){
		...
		$localQryResults = mysqli_query($conn, $sql);
		if(! ($localQryResults && mysqli_num_rows($localQryResults))){
...
/*817 - Combinatioins*/
	...
	static function getOpiateList(){
		static $qryResults;
		if(isset($qryResults)){
			return $qryResults;
		}
		$query ="SELECT * FROM `terms` t JOIN termspaths tp ON t.termID = tp.descendant WHERE tp.ancestor = 222 union SELECT * FROM `terms` t JOIN termspaths tp ON t.termID = tp.descendant WHERE tp.ancestor IN (SELECT tp.descendant FROM `terms` t JOIN termspaths tp ON t.termID = tp.descendant WHERE tp.ancestor = 222 ) ORDER BY `ancestor` ASC";
		$conn = databaseConnection::_getConnection();
		$qryResults = mysqli_query($conn, $query);
		if(! ($qryResults && mysqli_num_rows($qryResults))){
			unset($qryResults);
	...
	static function opiateListToArray(mysqli_result $source){
		$sortedArray = array();
		$namesArray = array();
		...
		while ($row = mysqli_fetch_assoc($source)){
			if(!($row && count($row))){
				...
				if($row['ancestor']== $row['descendant'] && length == 0){
					$namesArray[$row['termID']]['name']= $row['name'];
					$namesArray[$row['termID']]['url']= $row['name'];
				}
				if($row['ancestor']== 222 && $row['descendant']==222 && length == 0){
					$sortedArray[$row['termID']]= $row['termID'];
					
					if($row['ancestor']== 222 && length == 1){
					}				
				}
			}
		}
