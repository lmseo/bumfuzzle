...
require_once( $_SERVER['DOCUMENT_ROOT'] . '/includes/drug-master/drug-master-5-18-14/widgets/class.breadcrumb.php');
class page{
	private $dbTable;
	...
	public function setPageTemplate(){
		if(!($this->pageSource && mysql_num_rows($this->pageSource))){
		  return false;
			...
			if($row = mysql_fetch_assoc($this->pageSource) && isset($row['templateID'])){
