	...
	function scan_files($path,$afiles) 
	{
		...
		if(is_array($afiles)) {
			foreach($afiles as $cur) {
				...
				echo "          <li><a href=\"".$upath."/".rawurlencode($cur)."\" class=\"file\" title=\"$cur ";
				printf("[%.2f Kb]",$fsize);
	...
	function scan_dirs($path,$adirs) 
	{
		...
		if(is_array($adirs)) 
		{
			foreach($adirs as $cur) 
			{
				...
				echo "      <li>". "\n";
				echo "		  <a href=\"".$upath."/".rawurlencode($cur)."/\" class=\"folder\" title=\"$items items\">" . ucwords ($cleaner->clean_dir($cur)) . "</a>". "\n";
