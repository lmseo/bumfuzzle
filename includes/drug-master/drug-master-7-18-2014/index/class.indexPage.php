...
class indexNewsletterPage extends page{
	...
	public function _setIndexContent(){	
		if(!($this->indexSource && mysqli_num_rows($this->indexSource))){
		  return false;
		}
		else{
			$this->indexMainContent->_setMainColumnHTML();
			$i=0;
			while($row = mysqli_fetch_assoc($this->indexSource)){
				if($i !=  mysqli_num_rows($this->indexSource) && $i !=  0 ) {
					$this->indexMainContent->columnGroupHTML(0 , $row);
				}
				else if($i ==  0){
					$this->indexMainContent->columnGroupHTML(1 , $row);
				}
				else if($i ==  mysqli_num_rows($this->indexSource)-1) {
					$this->indexMainContent->columnGroupHTML(2 , $row);
				}
				  $i++;
			}
			mysqli_data_seek($this->indexSource, 0);
		}
		return true;
	}
