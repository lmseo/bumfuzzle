...
class opiatePage extends page{
		...
		if(!($this->randomRelatedTestimonial && mysql_num_rows($this->randomRelatedTestimonial))){
			...
			while ($row = mysql_fetch_assoc($this->randomRelatedTestimonial)){
				if(!($row && count($row))){
					...
					$contentHTML->set('text', substr($row['content'], 0, 400).'...');
	...
	public function printContactInformation(){
		$newToHTML = new toHTML();
		$newToHTML->setLinkArrayforAdditionalBox('https://www.opiates.com/contact/', 'Contact the Waismann Method', 'Email Us');
